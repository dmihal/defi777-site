...
import FarmerTokenFactoryABI from '../../constants/abis/FarmerTokenFactory.json';
import FarmerToken from './FarmerToken';
...
const scanForWrappers = async (web3: Web3, network: number, cb: (wrapper: string) => void) => {
  const { address, created } = contracts[network].FarmerTokenFactory;
  const contract = new web3.eth.Contract(FarmerTokenFactoryABI as any, address);
...
interface FarmerTokenFactoryProps {
  web3: Web3;
  network: number;
}

const FarmerTokenFactory: React.FC<FarmerTokenFactoryProps> = ({ web3, network }) => {
  ...
  return (
    <div>
      <h2>Farmer Token Factory</h2>

      <div>
        {wrappers.map((wrapper: string) => <FarmerToken address={wrapper} web3={web3} key={wrapper} />)}
      </div>
...
export default FarmerTokenFactory;
